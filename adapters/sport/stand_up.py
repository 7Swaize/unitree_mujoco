...
class StandUp(SportAdapter):
    ...
    def execute(self) -> None:
        ...
        while (runtime < execution_time_s):
            ...
            for i in range(12):
                self._cmd.motor_cmd[i].q = phase * self._stand_up_join_pos[i] + (1 - phase) * self._stand_up_join_pos[i]
                self._cmd.motor_cmd[i].kp = phase * 50.0 + (1 - phase) * 20.0
