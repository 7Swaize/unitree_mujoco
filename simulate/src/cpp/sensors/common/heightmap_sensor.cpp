...
mjtNum HeightmapSensor::CastGroundRay(const mjModel* m, mjData* d, double world_x, double world_y) const { 
    mjtNum start[3] = {world_x, world_y, d->xpos[3 * base_body_id_ + 2]};
    const mjtNum dir[3] = {0.0, 0.0, -1.0};

    mjtNum dist = mj_ray(m, d, start, dir, kGeomGroupMask.data(), 1, -1, nullptr);
...
void HeightmapSensor::Scan(const mjModel* m, mjData* d) {
    base_x_ = d->xpos[3 * base_body_id_ + 0];
    base_y_ = d->xpos[3 * base_body_id_ + 1];

    const mjtNum* R = d->xmat + 9 * base_body_id_;
